...
export default class SearchControl extends openlayers.control.Control {
  ...
  _convertGeospatialCoordinateFormat(inputString) {
    ...
    if (inputString.match(dmsPattern)) {
      var latitude = this.dmsToDd(RegExp.$1, RegExp.$2, RegExp.$3, RegExp.$4)
      var longitude = this.dmsToDd(RegExp.$5, RegExp.$6, RegExp.$7, RegExp.$8)
      ...
      var location = this.mgrsToDd(RegExp.$1, RegExp.$2, RegExp.$3, RegExp.$4, RegExp.$5, RegExp.$6)


      return this._convertGeospatialCoordinateFormat(location)
    }
    else { return false }
  }
    ...
    mgrsToDd (one, two, three, four, five, six)
    {
      var zone = one
      var zdl = two
      var c1 = three
      ...
      var u={}
      var ok=this.MGRStoUTM(zone,zdl,c1,c2,E,N,u)
      ...
      this.UTMXYToLatLon (x, y, u.zone, southhemi, latlon)

      var lat = ( this.RadToDeg (latlon[1]))
      var lon = ( this.RadToDeg (latlon[0]))
    ...
    UTMtoZDL(E,N)
    {
      ...
      if(N<=this.ZoneLatToN(84,E))
      {
        for(d=72;d>=-72;d-=8)
        {
          if(this.ZoneLatToN(d,E)<N)return this.UTMzdl(d+4)
        }
      }
     ...
     UTMtoMGRS(zone,E,N,d,u)
    {
      ...
      this.MGRSdigits(E,e,d)
      this.MGRSdigits(N,n,d)

      var E100=e.x100,N100=n.x100

      this.MGRSletters(zone,E100,N100,u)
     ...
     MGRStoUTM(zone,zdl,c1,c2,er,nr,u)
    {
      ...
      var approxN=this.zdlMedianLat(zdl)*100/90 // approx median northing of zdl in units of 100km
